...
export default function SupplierLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: "Inloggen mislukt", description: data.error, variant: "destructive" });
        return;
      }
      ...
      <div className="max-w-md mx-auto py-12 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold">Inloggen als winkel</h1>
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Inloggen</CardTitle>
              <CardDescription>Toegang tot je winkel dashboard</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="password" type="password" placeholder="Je wachtwoord" value={password} onChange={(e) => setPassword(e.target.value)} />
                </div>
