...
- Huidige versie: **v4.18** (avatar feature: upload/verwijder profielfoto via POST/DELETE /api/auth/avatar; GET /api/users/:id/avatar; `AvatarUpload` widget in profile + supplier-dashboard; `UserAvatar` component in bid cards; `avatar_data text` DB kolom; sharp 400×400 JPEG resize)
